...
Reduce each row of a (R,C) matrix to its max -> (R,).
...
You edit the CUDA inside the row_max string below. The host function must stay
named `row_max` and keep its signature: torch::Tensor row_max(torch::Tensor)
"""
row_max = r'''#include <torch/extension.h>
