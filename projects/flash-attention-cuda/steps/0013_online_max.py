"""
Step 0013: online_max

Part 4 — Online Softmax Math
Elementwise running max: out[i] = max(m[i], x[i]).
...
You edit the CUDA inside the online_max string below. The host function must stay
named `online_max` and keep its signature: torch::Tensor online_max(torch::Tensor,torch::Tensor)
"""
online_max = r'''#include <torch/extension.h>
#include <cuda_runtime.h>

__global__ void online_max_kernel(const float* m, const float* x, float* out, int n) {
    // TODO: i = blockIdx.x*blockDim.x + threadIdx.x; if (i<n) out[i] = fmaxf(m[i], x[i]);
}
torch::Tensor online_max(torch::Tensor m, torch::Tensor x) {
    auto out = torch::empty_like(a); int n = a.numel();
    // TODO: launch online_max_kernel with (n+255)/256 blocks of 256 threads
    return out;
}
'''
