...
def _is_pristine(path, name):
    """True if the step file is still an untouched stub (body just raises
    NotImplementedError). Used so we never overwrite real work."""
    try:
        with open(path) as f:
...
def main():
    ...
    for i, (name, part) in enumerate(spec.STEPS, start=1):
        ...
        manifest_steps.append({
            "id": sid, "name": name, "part": part, "points": 10,
    ...
    manifest = {
        "name": "alphazero-connect4",
        "title": "AlphaZero on Connect-4 from Scratch",
