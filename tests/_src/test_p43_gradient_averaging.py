...
from p43_gradient_averaging import *

def test_p43_gradient_averaging():
    ...
    with step("each replica's grad equals the mean across replicas"):
        for params_tuple, mean_g in zip(
            zip(*[list(r.parameters()) for r in replicas]),
            expected,
        ):
            for p in params_tuple:
                assert torch.allclose(p.grad, mean_g, atol=1e-6)
