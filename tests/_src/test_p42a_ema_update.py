...
from p42a_ema_update import *

def test_p42a_ema_update():
    torch.manual_seed(0)
    model = nn.Linear(4, 2)
    ema = nn.Linear(4, 2)
    with torch.no_grad():
    ...
    with step('ema_update: ema = 0.9 * 0 + 0.1 * model'):
        assert torch.allclose(ema.weight, 0.1 * init_w, atol=1e-06)
        assert torch.allclose(ema.bias, 0.1 * model.bias.detach(), atol=1e-06)
