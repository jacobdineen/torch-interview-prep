...
from p76_gradient_checkpointing import *

def test_p76_gradient_checkpointing():
    ...
    out_ref = ref(x); out_ck = ck(x_ck)
    with step("checkpointed forward matches reference forward"):
        assert torch.allclose(out_ref, out_ck, atol=1e-6)
    out_ref.sum().backward(); out_ck.sum().backward()
    with step("input gradients match reference"):
        assert torch.allclose(x.grad, x_ck.grad, atol=1e-6)
    with step("parameter gradients per-block match reference"):
        for p_ref, p_ck in zip(ref.parameters(), ck.parameters()):
            assert torch.allclose(p_ref.grad, p_ck.grad, atol=1e-6)
